    ...
    crawlSite(siteUrl, {
      ...
      onPageCrawled: async (result) => {
        ...
        await serviceClient.from("crawled_pages").insert({
          ...
          internal_link_urls: result.internalLinkUrls,
          external_link_urls: result.externalLinkUrls,
          ...
          url_depth: result.urlDepth,
          is_https: result.isHttps,
          has_mobile_viewport: result.hasMobileViewport,
...
export async function getCrawlResults(
  ...
  try {
    ...
    const results: CrawlResult[] = (data || []).map((row) => ({
      ...
      internalLinkUrls: row.internal_link_urls || [],
      externalLinkUrls: row.external_link_urls || [],
      ...
      urlDepth: row.url_depth || 0,
      isHttps: row.is_https ?? true,
      hasMobileViewport: row.has_mobile_viewport ?? false,
      openGraph: row.open_graph || { ogTitle: null, ogDescription: null, ogImage: null, ogType: null, ogUrl: null },
      schema: row.schema_data || { types: [], hasProduct: false, hasFAQ: false, hasArticle: false, hasLocalBusiness: false, hasOrganization: false },
      hreflangTags: row.hreflang_tags || [],
      lastModified: row.last_modified,
      contentHash: row.content_hash || "",
      hasAuthorInfo: row.has_author_info ?? false,
      hasDatePublished: row.has_date_published ?? false,
    }));
